...
- **搜索+详情双接口模式** (`media_platform/douyin/core.py`)：搜索后自动调用视频详情接口补全作者数据，获取粉丝数、作者获赞等完整信息
- **稳定版保留** (`media_platform/douyin/core_v1.py`)：纯搜索模式（无详情接口调用），速度更快，作为备用
- **版本切换**：修改 `media_platform/douyin/__init__.py` 第 21 行 import 即可切换
  - `from .core import DouYinCrawler` → 新版（完整数据）
  - `from .core_v1 import DouYinCrawler` → 稳定版（快速）
- **Excel 导出** (`export_excel.py`)：一键导出 SQLite 数据到 Excel，自动过滤图文、关键词精确匹配
- **新增数据字段**：粉丝数 (`follower_count`)、作者获赞 (`total_favorited`)、播放量 (`play_count`)
- **关闭昵称脱敏**：昵称和 UID 不再打码，存储原始值
...
- 数据库自动排除图文帖子 (aweme_type=68) 和标题不含关键词的记录
- Excel 列顺序：序号 → 昵称 → 粉丝数 → 作者获赞 → 视频标题 → 视频链接 → 互动数据 → 发布时间 → 关键词 → 时长
